...
namespace MultiPredicateTest {
    ...
    constexpr auto ImpactSulfur(float impact){
        return [impact](Res const& res){ return 1.0/(res.sulfur*impact);};
    }
    constexpr auto ImpactCarbon(float impact){
        return [impact](Res const& res){ return res.carbon*impact;};
    }
    constexpr auto ImpactPrice(float impact){
    ...
    TEST_CASE("sort") {
        ...
        constexpr auto const quality = MultiPred(std::multiplies<float>(),
                                       ImpactSulfur(2.0),
                                       ImpactCarbon(1.0),
                                       ImpactPrice(1.0));
